...
class BorrowRequest(BaseModel):
    duration_minutes: int | None = Field(default=None, ge=15, le=43200)
...
def _resolve_borrow_plan(now: datetime, data: BorrowRequest) -> tuple[str, int, str]:
    ...
    if planned_return_raw:
        ...
        if planned_dt < min_allowed:
            raise HTTPException(status_code=400, detail="Choisissez une date de retour au moins 15 minutes dans le futur")
        ...
        duration_minutes = int(max(15, (delta_seconds + 59) // 60))
